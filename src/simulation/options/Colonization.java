 ...
 */
public class Colonization extends Option
{
	Fleet fleet;
	Unit unit;
	...
	public void runAction()
	{
		...
		Simulator.instance().addOption(new ShipyardBuildEvent(location())); // TODO remove.
		
		// Can't colonize twice. The event is done.
		Simulator.instance().removeOption(this);
	}

	public static void check(Star s)
    {
        // We don't need previous colonization events.
        List<Option> existing = Simulator.instance().optionsForLocation(s);
        Iterator<Option> i = existing.iterator();
        while (i.hasNext())
        {
            Option e = i.next();
        ...
        Colony colony = s.getPlaceable(Colony.class);
        if (colony != null)
            return;

        // Are there still fleets in orbit?
        List<Fleet> fleets = s.getFleets();
        if (fleets.isEmpty())
            return;
        ...
        if (canColonize != null)
        {
            Colonization c = new Colonization(s, canColonize, toSpend);
            Simulator.instance().addOption(c);
        }
    }
}
